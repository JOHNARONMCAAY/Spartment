...
describe("Billing Service", () => {
  ...
  it("should return the billing summary correctly", async () => {
    ...
    // Assert
    expect(
      result.summary.electricity
    ).toBe(850);

    expect(
      result.summary.water
    ).toBe(220);

    expect(
      result.summary.totalUtilities
    ).toBe(1070);
  });
  ...
  it("should return the rent statements correctly", async () => {
    ...
    // Assert
    expect(
      result.rentStatements
    ).toHaveLength(2);

    expect(
      result.rentStatements[0].period
    ).toBe("May 2026");

    expect(
      result.rentStatements[0].status
    ).toBe("Paid");
  });
  ...
  it("should return the utility statements correctly", async () => {
    ...
    // Assert
    expect(
      result.utilityStatements
    ).toHaveLength(2);

    expect(
      result.utilityStatements[0].electricity
    ).toBe(850);

    expect(
      result.utilityStatements[0].water
    ).toBe(220);

    expect(
      result.utilityStatements[0].total
    ).toBe(1070);
  });
